...
RUN cd /app && sed '/"local":/s/.*/    "local": "live-server --host=0.0.0.0 --port=8080 --cors dist\/",/' package.json

RUN cd /app && npm i && npm run build
CMD cd /app && \
  sed "/\"Authorization\":/s/.*/            \"Authorization\": \"Bearer $API_KEY\",/" assets/js/scripts.js && \
  npm run local
